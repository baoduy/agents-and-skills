...
Ask the user which of `workspace`, `agents`, `squads`, `projects`, `autopilots`, `labels`, `properties` they want to import. **Default is `agents,squads`** — `workspace`, `projects`, `autopilots`, `labels`, and `properties` each require explicit opt-in.

`workspace` overwrites the **destination workspace's own settings** — its name, description, context, and issue prefix — with the bundle's. Two consequences to state plainly before the user opts in: the workspace may be **renamed**, and `--workspace <name>` is how this script addresses it, so a re-run needs the NEW name; and changing the **issue prefix** renumbers every issue key at the destination. The logo is in the bundle but cannot be applied (no CLI setter), and the repo registry is never touched.
...
- If `leadUnresolved`, `priorityUnsupported`, or `squadsSkipped` is non-empty, surface each entry verbatim as an "applied best-effort; adjust in the UI" note — e.g. "NOTE: applied best-effort; adjust in the UI — `<entry>`."
- If `resourcesMachineBound` is non-empty, surface every `<project-title>:<resource-type>` entry verbatim with: "NOTE: the following project resources were recreated verbatim, but their `resource_ref` names a daemon from the SOURCE machine — re-point each at a destination daemon in the Multica UI: `<entry>`."
- If `agentRuntimeSkillsDisabled` is non-empty, surface every `<agent-name>:<skill-name>` entry verbatim with: "NOTE: the following runtime-provided skills were switched OFF at the source but arrive ENABLED here — the multica CLI has no command to re-disable them (`agent skills` manages workspace skill assignments only). Switch each off in the Multica UI: `<entry>`."
- If `workspaceSettingsApplied` is true, say so: "Applied the bundle's workspace settings (name, description, context, issue prefix). The logo and the repo registry were not touched."
- If `workspaceRenamedFrom` is non-null, surface it with: "WARNING: the destination workspace was RENAMED from `<workspaceRenamedFrom>` to its bundled name. `--workspace` addresses a workspace by name, so use the NEW name for any re-run of this import."
- If `workspaceLogoUnsupported` is non-empty, surface it with: "NOTE: the workspace logo is in the bundle (`workspace/workspace.avatar.<ext>`) but the multica CLI has no command to set it — upload it in the Multica UI: `<workspace-name>`."
- **If `skillsEmptySkipped` is non-empty, say so first.** Surface every skill name verbatim with: "WARNING: the following bundle skills have an EMPTY `SKILL.md` and were SKIPPED rather than written over the destination — the bundle is broken, not the destination. Re-export it and re-run the import: `<skill-name>`."
